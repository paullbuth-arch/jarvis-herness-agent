...
import { Permission } from "@jarvis/schema/permission"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["jarvis.json", "jarvis.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".jarvis", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".jarvis")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".jarvis").toReversed()
    ...
    // global config, project files, then `.jarvis` files.
