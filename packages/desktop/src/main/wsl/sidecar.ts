...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export JARVIS_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export JARVIS_CLIENT=desktop",
    `export JARVIS_SERVER_USERNAME=${shellEscape(username)}`,
    `export JARVIS_SERVER_PASSWORD=${shellEscape(password)}`,
