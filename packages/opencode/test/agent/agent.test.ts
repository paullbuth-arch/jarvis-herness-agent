...
it.instance("plan agent denies edits except .jarvis/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".jarvis/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".jarvis", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.JARVIS_TEST_HOME
      process.env.JARVIS_TEST_HOME = test.directory
