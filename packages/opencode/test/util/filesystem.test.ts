...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "jarvis.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "jarvis.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jarvis.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jarvis.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["jarvis.json", "jarvis.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "jarvis.json"),
        path.join(tmp.path, "jarvis.jsonc"),
        path.join(project, "jarvis.json"),
        path.join(project, "jarvis.jsonc"),
      ])
    })
  })
