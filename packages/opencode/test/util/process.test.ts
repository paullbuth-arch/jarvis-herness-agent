...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.JARVIS_TEST ?? "")'), {
      env: {
        JARVIS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "JARVIS_TEST_SHELL"], {
      shell: true,
      env: {
        JARVIS_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("JARVIS_TEST_SHELL=ok")
  })
