...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/jarvis.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/jarvis.jsonc is not valid JSON(C): Unexpected token",
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/jarvis.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/jarvis.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Or check your config (jarvis.json) provider/model names",
    ].join("\n")
