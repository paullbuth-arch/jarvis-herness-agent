...
const TUI_SCHEMA_URL = "https://jarvis.wq7036/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from jarvis.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "jarvis"),
    ...(await Filesystem.findUp(["jarvis.json", "jarvis.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "jarvis"))
  }
  if (Flag.JARVIS_CONFIG) files.push(Flag.JARVIS_CONFIG)
