...
import { serviceUse } from "@jarvis/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@jarvis/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@jarvis/core/v1/config/console-state"
import { FSUtil } from "@jarvis/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@jarvis/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@jarvis/core/v1/config/config"
import { RemoteAuthError } from "@jarvis/core/v1/config/error"
import { ConfigPermissionV1 } from "@jarvis/core/v1/config/permission"
...
import { Npm } from "@jarvis/core/npm"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://jarvis.wq7036/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://jarvis.wq7036/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.JARVIS_CONFIG && !Flag.JARVIS_CONFIG_DIR && !Flag.JARVIS_CONFIG_CONTENT) {
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://jarvis.wq7036/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "JARVIS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://jarvis.wq7036/config.json"
        ...
        if (Flag.JARVIS_CONFIG) {
          yield* merge(Flag.JARVIS_CONFIG, yield* loadFile(Flag.JARVIS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.JARVIS_CONFIG })
        }
        ...
        if (Flag.JARVIS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from JARVIS_CONFIG_DIR", { path: Flag.JARVIS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".jarvis") || dir === Flag.JARVIS_CONFIG_DIR) {
            for (const file of ["jarvis.json", "jarvis.jsonc"]) {
          ...
          // Auto-discovered plugins under `.jarvis/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.JARVIS_CONFIG_CONTENT) {
          const source = "JARVIS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.JARVIS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from JARVIS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["JARVIS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("JARVIS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.JARVIS_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.JARVIS_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("JARVIS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.JARVIS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.JARVIS_DISABLE_PRUNE) {
