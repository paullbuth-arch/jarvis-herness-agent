...
import { ConfigMCPV1 } from "@jarvis/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in jarvis.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .jarvis/ subdirectory too)
  const candidates = [path.join(baseDir, "jarvis.json"), path.join(baseDir, "jarvis.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".jarvis", "jarvis.json"), path.join(baseDir, ".jarvis", "jarvis.jsonc"))
  }
  ...
  // Default to jarvis.json if none exist
  return candidates[0]
}
